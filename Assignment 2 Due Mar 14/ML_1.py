...
for index,i in enumerate(num_unlabeledPlot):
    ...
    for i in range(n):    #run 40 times to calculate 40 error rates
        ...
        '''Semi-supervised _ Self Learning with prior probabilities'''
        priorProba = []
        priorProba.append(float(12332)/float(19020))
        priorProba.append(float(19020-12332)/float(19020))
        ...
        for i in range(num_unlabeledPlot[index]/2):
                ...
                clf = LinearDiscriminantAnalysis(priors=priorProba)
                clf.fit(X_l, y_l)
        ...
        X_u2 = X[num_labeled1:]
        y_u2 = y[num_labeled1:]
        y_u_pred2 = clf.predict(X_u2)
        accuracy2 = accuracy_score(y_pred=y_u_pred2, y_true=y_u2)
        error3.append(1-accuracy2)

        ###### SS-Clustering ######
        X_u = X[25:25 + num_unlabeledPlot[index]]
        y_u = y[25:25 + num_unlabeledPlot[index]]
...
plt.legend([line_1,line_2, line_3, line_4], ['LDA Supervised Learning','Semi-supervised Self Learning with Prior Probabilities','Semi-supervised Self Learning', 'Semi-supervised Clustering'])
plt.show()
